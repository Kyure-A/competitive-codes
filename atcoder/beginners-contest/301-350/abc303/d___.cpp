  ...
  for (int i = 0; i < (int) encoded_string.size(); ++i)
      ...
      if (encoded_string[i].first == 'a')
	{
	  if (!isCapsLocked)
	    ...
	    {
	      if (encoded_string[i].second * x + z < encoded_string[i].second * y)
		{
		  isCapsLocked = !isCapsLocked;
		  time += encoded_string[i].second * x + z;
		}

	      else if (encoded_string[i].second * x + z > encoded_string[i].second * y)
		{
		  time += encoded_string[i].second * y;
		}
	    }
	}
