...
signed main ()
{
  ...
  for (int i = 0; i < n; ++i)
      {
	string s, t; cin >> s >> t;
	if (!convertID.count(s)) {convertID[s] = now_id; now_id++;}
	if (!convertID.count(t)) {convertID[t] = now_id; now_id++;}
	if (tree.same(convertID[s], convertID[t])) answer = "No";
	tree.merge(convertID[s], convertID[t]);
      }
