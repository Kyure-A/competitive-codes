...
signed main ()
{
  ...
  set<int> components_parent;
  vector<set<int>> components(n + 1);
  
  for (int i = 0; i < m; ++i)
    {
      edge[tree.root(u[i])]++;
      components[tree.root(u[i])].insert(u[i]);
      components[tree.root(v[i])].insert(v[i]);
      components_parent.insert(tree.root(u[i]));
    }
  ...
  if (m == 0)
    ...
    {
      for (auto comp : components_parent)
	{
	  if (!(components[comp].size() == edge[comp]))
	    {
	      cout << "No" << endl;
	      return 0;
	    }
