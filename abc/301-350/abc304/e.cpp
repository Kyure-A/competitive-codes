  ...
  for (int i = 0; i < k; ++i)
    {
      int x, y; cin >> x >> y;
      x--; y--;
      st.insert({tree.root(x), tree.root(y)});
      st.insert({tree.root(y), tree.root(x)});
    }
  ...
  while (query--)
    {
      int p, q; cin >> p >> q;
      p--; q--;
      if (st.count({tree.root(p), tree.root(q)}) or st.count({tree.root(q), tree.root(p)})) cout << "No" << endl;
      else cout << "Yes" << endl;
    }
