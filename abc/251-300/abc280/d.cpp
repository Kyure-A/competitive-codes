...
signed main ()
{
  ...
  factor = fast_factorize::factorize(k);
  sort(factor.begin(), factor.end(), greater<long long>());
  ...
  if (factor[0] % 2 == 0)
	...
	{
	  if (factor[i] == 2) count_2++;
	  else if (factor[i] != 2) break;
	}
      ...
      for (int i = 1; ; ++i)
	{
	  ...
	  if (k < pow(2, i + 1))
	    {
	      if (count_2 > sum) cout << pow(2, count_2) << endl;
	      else cout << pow(2, sum) << endl;
	      break;
	    }
	}
